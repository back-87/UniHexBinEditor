...
struct UniHexBinEditor: View {
    ...
    init(_ initialString : String) {
        viewModel = UHBEViewModel(initialString)
    }
    
    init(_ initialData : Data) {
        viewModel = UHBEViewModel(initialData)
    }
    
    init() {
        viewModel = UHBEViewModel()
    }
    ...
    var body: some View {
        VStack {
            HStack {
                Spacer()
                VStack {
                    UniHexBinEditorRadioButtons (selectedEditMode: $currentEditingMode, { selected in
            ...
            .onChange(of: stringValue, perform: { newValue in
                ...
                switch (currentEditingMode) {
                    case BinaryEditMode.unicode:
                        viewModel.workingData = stringValue.data(using: .utf8)
                    case BinaryEditMode.hex:
                        viewModel.workingData = Data(hexString: stringValue, expectSeparatedBytes: expectSeparatedBytes)
